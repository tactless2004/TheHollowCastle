...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        if(Physics.Raycast(sphereCastOrigin.position, playerMove.facing, out RaycastHit hit, maxPickupDistance))
        {
    ...
    private void Update()
    {
        Debug.DrawRay(sphereCastOrigin.position, playerMove.facing * maxPickupDistance, Color.green);
        if(Physics.Raycast(sphereCastOrigin.position, playerMove.facing, out RaycastHit hit, maxPickupDistance))
        {
            if(hit.collider.TryGetComponent(out PickupItem pickupItem))
            {
